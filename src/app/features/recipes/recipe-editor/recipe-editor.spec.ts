...
describe('RecipeEditor', () => {
  ...
  const recipe: Recipe = {
    ...
    brewer: 'Guaja',
    untappdUrl: '',
    equipmentProfileId: pilot.id,
    mashProfileId: 'mash',
    carbonationProfileId: 'carbonation',
    fermentationProfileId: 'fermentation',
    glasswareId: 'teku',
    styleId: 'style-neipa',
    batchVolumeL: 20,
    efficiencyPercent: 72,
    boilVolumeL: 24,
    malts: [
      { maltId: 'pale', amountKg: 4, notes: 'Base' },
      { maltId: 'oats', amountKg: 1, notes: '' },
    ],
    hops: [
      {
        type: 'lúpulo',
    ...
    waterProfileId: 'neipa-water',
    waterAdditions: [{ saltId: 'gypsum', name: 'Gypsum', amountG: 3 }],
    ...
    fermentationSteps: [
      { stage: 'primaria', startDay: 0, durationDays: 10, temperatureC: 19, notes: '' },
    ],
    dryHop: { enabled: true, days: 3, temperatureC: 18 },
    packaging: { maturationDays: 14, carbonationVolumes: 2.4, method: 'Botella' },
    notes: '',
    version: 2,
  ...
  const catalogData = {
    ...
    yeasts: [
      {
        ...
        temperatureMin: 18,
        temperatureMax: 23,
        sensoryProfile: 'Frutal',
      },
    ],
    adjuncts: [
      { id: 'servo', name: 'Servomyces', format: 'polvo', category: 'nutriente', description: '' },
    ],
    ...
    mashProfiles: [
      { id: 'mash', mashTempC: 65, mashTimeMin: 60, mashOutTempC: 76, mashOutTimeMin: 10 },
    ],
  ...
  let calculator: {
    ...
    compareToStyle: ReturnType<typeof vi.fn>;
    calculateHopIbu: ReturnType<typeof vi.fn>;
  };
  ...
  it('gestiona filas, porcentajes, orden y sincronización de lúpulos', () => {
    ...
    expect(editor.fermentationStepsArray.at(1).value).toMatchObject({
      stage: 'cold crash',
      durationDays: 2,
      temperatureC: 4,
    });
    ...
    const stopped = vi.fn();
    editor.startRowDrag(
      { dataTransfer: {} as DataTransfer, stopPropagation: stopped } as unknown as DragEvent,
      editor.maltsArray,
      0,
    );
  ...
  it('aplica perfiles de agua, equipo, macerado, fermentación y carbonatación', () => {
    ...
    expect(editor.form.controls.waterTreatment.value).toMatchObject({
      calcium: 144,
      chloride: 180,
      mashPh: 5.2,
    });
    ...
    expect(editor.form.controls.fermentation.value).toMatchObject({
      primaryDays: 8,
      secondaryDays: 2,
    });
    expect(editor.form.controls.packaging.controls.maturationDays.value).toBe(10);
    editor.applyCarbonationProfile('carbonation');
    expect(editor.form.controls.packaging.value).toMatchObject({
  ...
  it('calcula resumen visual y escala como modificación o como copia', () => {
    ...
    expect(notifications.success).toHaveBeenCalledWith(
      expect.stringContaining('guardada como copia'),
    );
  ...
  it('guarda, duplica y comunica fallos de validación y persistencia', () => {
    ...
    editor.duplicate();
    expect(store.saveRecipe).toHaveBeenLastCalledWith(
      expect.objectContaining({ name: 'Copia de NEIPA', version: 1, image: undefined }),
    );
  });
  ...
  it('valida imágenes y procesa progreso, respuesta y errores de subida', () => {
    const invalid = {
      files: [new File(['x'], 'label.gif', { type: 'image/gif' })],
      value: 'label.gif',
    };
    ...
    api.uploadRecipeImage.mockReturnValueOnce(
      of(
    ...
    const valid = {
      files: [new File(['image'], 'one.png', { type: 'image/png' })],
      value: 'one.png',
    };
    ...
    editor.uploadImage({
      target: { files: [new File(['image'], 'big.png', { type: 'image/png' })], value: 'big.png' },
    } as unknown as Event);
  ...
  it('resuelve datos del catálogo y abre la impresión de receta', () => {
    const print = vi.spyOn(window, 'print').mockImplementation(() => undefined);

    expect(editor.catalogName(catalogData.malts, 'pale')).toBe('Pale Ale');
    expect(editor.catalogName(catalogData.malts, 'missing')).toBe('—');
    expect(editor.catalogMaltSrm(catalogData.malts, 'pale')).toBe(3);
    editor.printRecipe();
