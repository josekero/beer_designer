...
@Component({selector:'app-account-profile',imports:[ReactiveFormsModule],templateUrl:'./account-profile.html',styleUrl:'./account-profile.scss'})
export class AccountProfile{
  ...
  readonly avatars=['amber-pint','hazy-ipa','stout','red-ale','cider','teku'];readonly saving=signal(false);readonly avatarVersion=signal(Date.now());
  readonly profile=this.fb.nonNullable.group({displayName:[this.auth.user()?.displayName??'',[Validators.required,Validators.minLength(2)]],avatarValue:[this.auth.user()?.avatarValue??'amber-pint']});
