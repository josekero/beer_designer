...
export class AuthService {
  ...
  login(email:string,password:string):Observable<ApplicationUser>{
    return this.http.post<ApplicationUser>('/api/auth/login',{email,password}).pipe(
      switchMap(user => this.initializeCsrf().pipe(map(() => user))),
      tap(user=>{this.user.set(user);this.ready.set(true);})
    );
  }
  register(email:string,password:string,displayName:string):Observable<ApplicationUser>{
    return this.http.post<ApplicationUser>('/api/auth/register',{email,password,displayName}).pipe(
      switchMap(user => this.initializeCsrf().pipe(map(() => user))),
      tap(user=>{this.user.set(user);this.ready.set(true);})
    );
  }
  ...
  private initializeCsrf():Observable<void>{
    return this.http.get('/api/auth/csrf').pipe(map(()=>undefined));
  }
}
